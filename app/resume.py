...
from app.schema import Award, Experience, ResumeState, Skills, migrate_legacy_resume_data
...
def collect_resume_info(
    ...
    # 旧客户端可能仍发送 basic_info.university/major；在合并前转成规范字段，
    # 这样“修正学校/专业”能够覆盖旧值，而不是被完整状态迁移逻辑忽略。
    patch = migrate_legacy_resume_data(patch, prefer_legacy=True)
...
def check_missing_fields(state: ResumeState | Mapping[str, Any] | str | None) -> dict[str, Any]:
    ...
    # missing_fields 与 validation_errors 是模板生成的硬门槛；quality_questions
    # 只用于引导用户提高表达质量，不应阻塞已经完整的简历生成。
...
def build_template_context(state: ResumeState | Mapping[str, Any] | str | None) -> dict[str, str]:
    ...
    return {
        ...
        "education_header": (
            f"{education.school or '待补充'} "
            f"{education.college or '待补充'} "
            f"{education.major or '待补充'}专业  "
        ),
